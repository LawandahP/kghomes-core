# Generated by Django 4.2.4 on 2024-02-04 17:20

from django.db import migrations, models
import utils.utils


class Migration(migrations.Migration):

    dependencies = [
        ('leases', '0003_alter_lease_id'),
    ]

    operations = [
        migrations.AlterField(
            model_name='invoice',
            name='status',
            field=models.CharField(choices=[('Fully Paid', 'Fully Paid'), ('Unpaid', 'Unpaid'), ('Partially Paid', 'Partially Paid'), ('Due', 'DUE'), ('Overdue', 'Overdue')], default='Unpaid', max_length=100),
        ),
        migrations.AlterField(
            model_name='lease',
            name='id',
            field=utils.utils.CustomUUIDField(default=utils.utils.CustomUUIDField.generate_uuid, max_length=20, primary_key=True, serialize=False, unique=True),
        ),
    ]
